...
V26 核心变更：第 7 章（DeepSeek 执行指令）与第 1-6 章正文完全对齐，取代 V22 的“中间旧规则 + 末尾追加修正”结构（见第 7 章开头说明）；第 5 章错误码表随本版封版 —— 此后**不再引入 404**，资源不存在统一走业务码（见 5.1）。技术设计不变。
V25：（说明）现有代码、文档与 git 历史中均查不到任何 V25 引用，其变更内容已不可考，故不臆造补写；如需追溯，请查 V24 与 V26 之间的提交记录。
V24 核心变更：用户级 Token version 机制由 P1 提升为 P0（见 1.1，成本低、安全亮点、答辩演示价值高）。技术设计不变。
